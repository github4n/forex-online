<template>
	<div id="codeDialog" v-if="isshow">
		...
		<div class="page_cont">
			...
			<div class="code_box">
				<input type="number" autofocus class="fl" id="code-input" placeholder="图形验证码" v-model="code" />
							...
							this.$http.post(this.PATH + '/sms',{emulateJSON: true},{
								...
								if(data.success == true){
									if(data.code == 1){
										this.$refs.dialog.isShow = true;
										this.msg = '发送成功';
										this.$parent.isSendCode = true;
										this.$parent.time = 60;
										this.$parent.timeInt = setInterval(function(){
											this.$parent.time -= 1;
										}.bind(this),1000);
										setTimeout(function(){
											this.$parent.isSendCode = false;
											this.$parent.time = 0;
											clearInterval(this.$parent.timeInt);
											this.$parent.timeInt = null;
										}.bind(this),60000);
