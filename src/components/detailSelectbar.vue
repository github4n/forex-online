		...
		watch: {
			...
			$route:function(){
				if(this.$route.name == 'orderdetail'){
					if(this.$route.query.type == 1||this.$store.state.menuType == 5){
						this.sel('交易中心');
					}else if(this.$store.state.menuType == 2){
						this.sel('分时');
					}else if(this.$store.state.menuType == 1){
						this.sel('闪电图');
					}else if(this.$store.state.menuType == 3){
						this.sel('K线');
					}else if(this.$store.state.menuType == 4){
						this.sel('盘口');
					}
				}
			}
...
//				this.$store.state.isshow.isklineshow = true;
			...
			sel: function(e) {
				var tex = '';
				if(e == '交易中心'||e == '分时'||e == '闪电图'||e == '盘口'||e == 'K线'){
					tex = e;
				}else{
					tex = e.currentTarget.innerText;
				}
				switch(tex) {
					case '闪电图':
						this.$store.state.menuType = 1;
...
//						var tradeConfig =this.$store.state.market.tradeConfig;
		...
		mounted: function() {
			this.$store.state.isshow.bottomshow = false;
			if(this.$store.state.menuType == 5){
				this.sel('交易中心');
			}else if(this.$store.state.menuType == 2){
				this.sel('分时');
			}else if(this.$store.state.menuType == 1){
