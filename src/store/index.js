...
var market = {
	...
	state: {
		...
		quoteConfig:{
			...
			url_real: "wss://quote.vs.com:9003",   //正式地址WSS
		...
		tradeConfig:{
			...
			// url_real : "ws://192.168.0.227:36999",  //测试地址 v2
			// url_real : "ws://101.132.27.158:6999",  //正式地址 v2
			// url_real: "wss://futurescompetitiontrade.vs.com:6103",//正式地址 WSS
			url_real: "ws://192.168.0.227:36999", //最新交易地址6.0 v2
			model : "1", // 实盘：0；	模拟盘：1
...
export default new Vuex.Store({
	...
	state: {
		...
		version: {},
		appVersion:'v1',
		menuType:2, //1：闪电图 2：分时图 3：k线图 4：盘口 5：交易中心 
		colorType:1,//涨跌颜色
		colorUp:'#dd2d4a',
		colorDown:'#3dcc85',
		openPage:0,
	...
	mutations: {
		...
		setklineoption2: function(state) {
		...
		//			volume.setOption(state.market.option2);
		//		},
		setfensoptionsecond2: function(state) {
			// var echarts = require('echarts/lib/echarts');
			var vol = [],
				price = [],
				time = [],
...
//					console.log('lastHeartBeatTimestamp:'+context.state.market.HeartBeat.lastHeartBeatTimestamp);
				...
				case 'OnRspLogin'://登录回复
					if(parameters.Code==0){
						...
						context.state.market.positionListCont = [];
						context.state.market.OnRspQryTradeDealListCont = [];
						context.state.market.orderListCont = [];
						context.state.market.OnRspOrderInsertEntrustCont = [];
						context.state.market.hasNostopLossList = [];
						context.state.market.hasYesstopLossList = [];
						context.state.market.conditionList = [];
						context.state.market.triggerConditionList = [];
						context.state.market.qryHoldTotalArr = [];
						context.state.market.OnRspOrderInsertOrderListCont = [];
						context.state.market.stopLossList = [];
						context.state.market.stopLossTriggeredList = [];

						context.state.loginStatus = 1;
						...
						context.state.loginStatus = 2;
						//清空本地交易登录信息
						localStorage.tradeUser = null;
					}
					setTimeout(function(){
						context.state.loginStatus = 0;
					},1000);
					break;
...
//					console.log('资金变化通知');
					...
					// 更新资金汇总信息
					context.dispatch('updateTotalAccount',parameters);
					// context.dispatch('afterSellAccount',parameters);
...
//						context.state.market.CacheHoldFloatingProfit.jCurrencyNoFloatingProfit[contractFloatingProfit.currencyNo]=0.0;
				...
				//更新账户盈亏
				context.dispatch('updateTotalAccount','hangqing');
		...
		//平仓后更新资产
		afterSellAccount:function(context,parameters){
			...
			for( var e in jCacheAccount){
				...
				// 今权益 = 今结存 + 浮盈
				var tmpTodayBalance = jCacheAccount[e].TodayAmount;
		...
		updateTotalAccount:function(context,parameters){
			...
			for( var e in jCacheAccount){
				...
				var floatingProfit = jCacheAccount[e].FloatingProfit;
				// context.state.market.CacheAccount.jCacheTotalAccount.FloatingProfit  += floatingProfit * jCacheAccount[e].CurrencyRate;
				context.state.market.CacheAccount.jCacheTotalAccount.FloatingProfit = 0;
				
				// // 今权益 = 今结存 + 浮盈
				if(parameters == 'hangqing'){
...
//					console.log(context.state.market.HeartBeat.oldHeartBeatTimestamp);
		...
		initTrade:function(context,params){
			...
			context.state.tradeSocket.onopen = function(evt){
				...
				if(context.state.tradeSocket.readyState==1){ //连接已建立，可以进行通信。
					if(params){
						context.state.tradeSocket.send(params);
					}else{
						if(JSON.parse(localStorage.getItem('tradeUser'))){
							context.state.market.tradeConfig.username = JSON.parse(localStorage.getItem('tradeUser')).username;
...
//				console.log(evt);
		...
		initQuoteClient: function(context) {
			if(context.state.quoteSocket){
				context.state.quoteSocket.close();
				context.state.market.Parameters = [];
			}
...
//								context.commit('updateTempdata', context.state.market.currentNo);
								// context.commit('setfensoptionsecond2');
...
//											context.state.market.jsonData.Parameters.Data.shift();
								...
								if(context.state.isshow.isklineshow == true) {
									...
									context.commit('setklineoption2');
