...
//			},
			...
			money: function(){
				if(this.money == ''){
					...
					if(this.money < 0){
						...
						if(this.money>this.operateMoney){
							//收取手续费
							this.poundage = ((this.money-this.operateMoney)*this.feeRate).toFixed(2);
							this.relmoney = (this.money-this.poundage).toFixed(2);
						}else{
							//不收
							this.poundage = 0;
		...
		methods: {
			//获取手续费费率
			getFeeRate(){
				this.$http.post(this.PATH + '/user/withdraw/feeRate', {emulateJSON: true},{
					headers: {
					...
					if(data.success == true){
						if(data.code == 1){
							this.feeRate = data.data.feeRate;
							this.operateMoney = data.data.operateMoney;
						}
					}else{
						switch (data.code){
							case '3':
