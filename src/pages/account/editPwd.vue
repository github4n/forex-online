	...
	<div id="editPwd">
		...
		<div class="page_cont">
			<div class="ipt_row">
				...
				<input type="text" id="phone" placeholder="请输入您的手机号" disabled v-model="showPhone" />
			</div>
			<div class="ipt_row">
...
<script>
	...
	export default{
		...
		methods: {
			...
			confirmEdit: function(){
				if(this.code == ''){
					...
					this.$children[0].isShow = true;
					this.msg = '请确认新密码';
				}else if(this.newPwd == this.userInfo.password){
					this.$children[0].isShow = true;
					this.msg = '新密码与旧密码一样';
					...
					//请求修改密码
					this.$http.post(this.PATH + '/user/security/update_loginPwd', {emulateJSON: true},{
						headers: {
							'token':  this.userInfo.token,
							'secret': this.userInfo.secret
						},
		...
		activated: function(){
			...
			this.showPhone = String(this.userInfo.username).substr(0,4) + '****' + String(this.userInfo.username).substr(-4);
		}
	}
</script>
