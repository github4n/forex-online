...
<script>
	...
	export default {
		...
		computed: {
			...
			detail() {
				this.cname = this.currentDetail.CommodityName;
				this.cnum = this.currentDetail.CommodityNo + this.currentDetail.MainContract;
...
//			this.$store.state.isshow.islightshow = false;
		...
		mounted: function() {
			// document.getElementById('orderdetail').addEventListener('touchstart',function(e){
...
//			this.$store.state.isshow.bottomshow = false;
		...
		beforeRouteEnter(to,from,next){
			if((from.path == '/')&&to.path == '/orderdetail'){
				next({path:'/home'})
			}else{
				next();
			}
