<template>
	<div id="agreement">
		<topbar v-if="$store.state.appVersion == 'v1'" title="操盘合作协议"></topbar>
		<topbar v-if="$store.state.appVersion == 'v2'" title="期货操盘协议"></topbar>
		<back></back>
		<cs title="客服"></cs>
		<div class="page_cont">
			<h3 id="newstitle"><span v-if="$store.state.appVersion == 'v1'">{{type}}</span><span v-if="$store.state.appVersion == 'v2'">期货</span>操盘委托协议</h3>
			...
			<p>住所地：</p>
			<p>鉴于</p>
			<p>1、甲方接受直达期货股份有限公司（以下简称“委托人”）委托，担任投资顾问，受托管理该资产。为更好管理资产，维护委托人权益，经委托人同意，甲方有意委托若干自然人提供<span v-if="$store.state.appVersion == 'v1'">{{type}}</span><span v-if="$store.state.appVersion == 'v2'">期货</span>投资策略服务；</p>
			<p>2、乙方有意接受甲方前述委托，申请提供该服务；</p>
			<p>3、丙方系本APP（下称“丙方应用”）的运营商，丙方具有应用运营及技术服务优势，可使乙方通过丙方应用申请提供前述服务。</p>
			<p>各方根据《期货交易管理条例》、《中华人民共和国合同法》等法律法规，经友好协商，订立本合作协议（下称“本协议”），供各方遵照执行。</p>
			<p>第一条 本协议成立及生效</p>
			<p>1、乙方可以通过丙方应用相应页面申请向甲方提供<span v-if="$store.state.appVersion == 'v1'">{{type}}</span><span v-if="$store.state.appVersion == 'v2'">期货</span>投资策略服务（下称“服务”），本协议各方确认，一旦乙方在丙方应用相关页面点击确认申请，则本协议在协议各方之间成立，且本协议于乙方按照本协议规定完成相应操盘保证金支付时生效。本协议各方应本着诚实信用原则按照本协议约定全面履行。</p>
			<p>2、根据本协议，在甲方与乙方之间形成甲方委托乙方提供<span v-if="$store.state.appVersion == 'v1'">{{type}}</span><span v-if="$store.state.appVersion == 'v2'">期货</span>投资策略服务法律关系，乙方申请行为并不在乙方与甲方、丙方或任何其他第三方之间形成借贷关系。</p>
			<p>第二条 合作方式</p>
			<p>1、 乙方以保证金方式取得甲方期货交易账户的操作权限，且乙方独立承担操盘的全部风险。甲方授权给乙方操盘的期货交易账户可交易金额为乙方保证金的一定比例。具体约定如下：</p>
			<p>乙方保证金 ：</p>
			<p>甲方提供(期货账户) 初始金额：</p>
			<p>2、  乙方可以使用上述期货交易账户内资金总额购买<span v-if="$store.state.appVersion == 'v1'">{{type}}</span><span v-if="$store.state.appVersion == 'v2'">期货</span>市场主流{{type}}品种，共计17种。盈利归乙方所有，亏损由乙方承担，即乙方对上述账户进行期货买卖等操作产生的一切风险和后果由乙方承担，与甲方无关。</p>
			<p>第三条 操盘规则</p>
			...
			<p>第四条 丙方的服务</p>
			<p>1、丙方在丙方应用相应页面（具体位置由丙方决定），推出“国际综合” 操盘项目（以下简称“项目”），相应具有<span v-if="$store.state.appVersion == 'v1'">{{type}}</span><span v-if="$store.state.appVersion == 'v2'">期货</span>操作经验的乙方注册成为丙方应用注册用户后可通过该页面申请提供<span v-if="$store.state.appVersion == 'v1'">{{type}}</span><span v-if="$store.state.appVersion == 'v2'">期货</span>投资策略服务。</p>
			...
			<p>第七条 操盘保证金</p>
			<p>1、乙方在支付操盘保证金后可向甲方提供相应的<span v-if="$store.state.appVersion == 'v1'">{{type}}</span><span v-if="$store.state.appVersion == 'v2'">期货</span>投资策略服务。</p>
