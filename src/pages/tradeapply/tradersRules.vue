<template>
	<div id="tradersRules">
		<topbar v-if="$store.state.appVersion == 'v1'" title="国际综合操盘细则"></topbar>
		<topbar v-if="$store.state.appVersion == 'v2'" title="期货操盘细则"></topbar>
		<back></back>
		<cs title="客服"></cs>
		<div class="page_cont">
			<h3>操盘流程：</h3>
			<p>①选择方案 → ②支付保证金 → ③获取操盘账号 → ④在指定软件里操盘 → ⑤终结方案、结算到账</p>
			<h3 v-if="$store.state.appVersion == 'v1'">操盘合约：</h3>
			<p v-if="$store.state.appVersion == 'v1'">可同时操盘17种国际期货的当期主力合约，分别为：富时A50、恒指期货、国际原油、迷你纳指、迷你道指、迷你标普、德国DAX、日经225、小恒指、美黄金、H股指数、小H股指数、美铜、美白银、小原油、迷你德国DAX指数、天然气。</p>
			<h3>交易时间： </h3>
